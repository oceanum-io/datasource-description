...
> **NERC vocabulary links**: hyperlink each Table 2/Table 3 variable name to its NERC standard-name page, e.g. `<a href="https://vocab.nerc.ac.uk/standard_name/sea_surface_wave_significant_height/" target="_blank">hs</a>`, opening in a new tab. The `standard_name` stored in the datasource is often Oceanum-adapted and 404s on NERC — **verify each URL returns 200** and correct to canonical CF (`below_8s_period`→`wind_wave`, `above_8s_period`→`swell_wave`, drop `_at_10m_above_ground_level`, `_mean_from_direction`→`_from_direction`). Leave variables with no NERC page (`fspr`, `qb`, `lm`, `pwlen*`, partition-4/quaternary, gridstats vars) as plain text. The full verified `variable → standard_name` map is in the `new-dataset-doc` skill (Step 5).
